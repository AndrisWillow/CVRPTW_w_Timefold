...
class Vehicle(JsonDomainBase):
    ...
    def arrival_time(self) -> Optional[datetime]:
        # If there are no visits, it arrives back home at departure_time
        # (or you can choose to set it to None too)
        if len(self.visits) == 0:
            return self.departure_time
        
        last_departure_time = self.visits[-1].departure_time
        if last_departure_time is None:
            # The chain isn't fully assigned or is inconsistent yet
            return None
